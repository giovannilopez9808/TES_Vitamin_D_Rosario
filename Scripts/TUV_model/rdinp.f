...
*=============================================================================*
      ...
      SUBROUTINE rdinp(intrct,inpfil,outfil,nstr,lat,lon,tmzone,iyear,
     $   imonth,iday,zstart,zstop,nz,wstart,wstop,nwint,tstart,tstop,nt,
     $   lzenit,alsurf,psurf,o3col,so2col,no2col,taucld,zbase,ztop,
     $   tauaer,ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,lirrad,
     $   laflux,lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,itfix,
     $   izfix,ims,slabel,imj,jlabel)
...
* choose input file
 ...
 12   CONTINUE
      ...
      IF(inpfil .EQ. 'defin1' .OR. inpfil .EQ. 'defin2' .OR.inpfil .EQ.'
     $   defin3') THEN
...
* read input file:
      ...
      CALL readin(inpfil,outfil,nstr,lat,lon,tmzone,iyear,imonth,iday,
     $   zstart,zstop,nz,wstart,wstop,nwint,tstart,tstop,nt,lzenit,
     $   alsurf,psurf,o3col,so2col,no2col,taucld,zbase,ztop,tauaer,
     $   ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,lirrad,laflux,
     $   lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,itfix,izfix,ns,
     $   ls,ims,slabel,nj,lj,imj,jlabel)
      CLOSE (kin)
...
************* check and report values:
 ...
 20   CONTINUE
      ...
      CALL chkval(aerror,okvalu,oknew,inpfil,outfil,nstr,lat,lon,tmzone,
     $   iyear,imonth,iday,zstart,zstop,nz,wstart,wstop,nwint,tstart,
     $   tstop,nt,lzenit,alsurf,psurf,o3col,so2col,no2col,taucld,zbase,
     $   ztop,tauaer,ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,
     $   lirrad,laflux,lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,
     $   itfix,izfix,ns,ls,ims,slabel,nj,lj,imj,jlabel)

      IF (.NOT. intrct .AND. okvalu) GO TO 99

      CALL write1(6,inpfil,outfil,nstr,lat,lon,tmzone,iyear,imonth,iday,
     $   zstart,zstop,nz,wstart,wstop,nwint,tstart,tstop,nt,lzenit,
     $   alsurf,psurf,o3col,so2col,no2col,taucld,zbase,ztop,tauaer,
     $   ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,lirrad,laflux,
     $   lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,itfix,izfix,ns,
     $   ls,ims,slabel,nj,lj,imj,jlabel)
      ...
      IF(.NOT. okvalu) THEN
         WRITE(*,*)'****** incorrect value for variable: ',aerror
         IF(.NOT. oknew) THEN
...
* change values
 ...
 40   CONTINUE
      CALL newval(avar,okfind,oktype,inpfil,outfil,nstr,lat,lon,tmzone,
     $   iyear,imonth,iday,zstart,zstop,nz,wstart,wstop,nwint,tstart,
     $   tstop,nt,lzenit,alsurf,psurf,o3col,so2col,no2col,taucld,zbase,
     $   ztop,tauaer,ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,
     $   lirrad,laflux,lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,
...
* (cannot overwrite default input files: defin1, defin2, defin3 )
      ...
      IF(finame .EQ. 'defin1' .OR. finame .EQ. 'defin2' .OR.finame .EQ.'
     $   defin3' .OR. finame .EQ. 'defin4') THEN
 ...
 75   CONTINUE
      ...
      CALL write1(20,savfil,outfil,nstr,lat,lon,tmzone,iyear,imonth,
...
***** done
 ...
 99   CONTINUE
      ...
      CALL write1(kout,inpfil,outfil,nstr,lat,lon,tmzone,iyear,imonth,
     $   iday,zstart,zstop,nz,wstart,wstop,nwint,tstart,tstop,nt,lzenit,
     $   alsurf,psurf,o3col,so2col,no2col,taucld,zbase,ztop,tauaer,
     $   ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,lirrad,laflux,
     $   lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,itfix,izfix,ns,
...
*=============================================================================*
      ...
      SUBROUTINE write1(iout,inpfil,outfil,nstr,lat,lon,tmzone,iyear,
     $   imonth,iday,zstart,zstop,nz,wstart,wstop,nwint,tstart,tstop,nt,
     $   lzenit,alsurf,psurf,o3col,so2col,no2col,taucld,zbase,ztop,
     $   tauaer,ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,lirrad,
     $   laflux,lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,itfix,
     $   izfix,ns,ls,ims,slabel,nj,lj,imj,jlabel)
...
* right-justify 6-character file names
 ...
 105  FORMAT('inpfil = ',5x,A6,3X,'outfil = ',5x,A6,3X,'nstr = ',2X,
     $I11)
 110  FORMAT('lat = ',3X,F11.3,3X,'lon = ',3X,F11.3,3X,'tmzone = ',
     $F11.1)
 115  FORMAT('iyear = ',1X,I11,3X,'imonth = ',I11,3X,'iday = ',2X,I11)
 120  FORMAT('zstart = ',F11.3,3X,'zstop = ',1X,F11.3,3X,'nz = ',4X,
     $I11)
 125  FORMAT('wstart = ',F11.3,3X,'wstop = ',1X,F11.3,3X,'nwint = ',1X,
     $I11)
 130  FORMAT('tstart = ',F11.3,3X,'tstop = ',1X,F11.3,3X,'nt = ',4X,
     $I11)
 135  FORMAT('lzenit = ',10X,L1,3X,'alsurf = ',F11.3,3X,'psurf = ',1X,
     $F11.1)
 140  FORMAT('o3col = ',1X,F11.3,3X,'so2col = ',F11.3,3X,'no2col = ',
     $F11.3)
 145  FORMAT('taucld = ',F11.3,3X,'zbase = ',1X,F11.3,3X,'ztop = ',2X,
     $F11.3)
 150  FORMAT('tauaer = ',F11.3,3X,'ssaaer = ',F11.3,3X,'alpha = ',1X,
     $F11.3)
 155  FORMAT('dirsun = ',F11.3,3X,'difdn = ',1X,F11.3,3X,'difup = ',1X,
     $F11.3)
 160  FORMAT('zout = ',2X,0pF11.3,3X,'zaird = ',1X,1pE11.3,3X,
     $'ztemp = ',1X,0pF11.3)
 165  FORMAT('lirrad = ',10X,L1,3X,'laflux = ',10X,L1,3X,'lmmech = ',
     $10X,L1)
 170  FORMAT('lrates = ',10X,L1,3X,'isfix = ',1X,I11,3X,'nms = ',3X,
     $I11)
 175  FORMAT('ljvals = ',10X,L1,3X,'ijfix = ',1X,I11,3X,'nmj = ',3X,
     $I11)
 180  FORMAT('iwfix = ',1X,I11,3X,'itfix = ',1X,I11,3X,'izfix = ',1X,
     $I11)
...
*=============================================================================*
      ...
      SUBROUTINE readin(inpfil,outfil,nstr,lat,lon,tmzone,iyear,imonth,
     $   iday,zstart,zstop,nz,wstart,wstop,nwint,tstart,tstop,nt,lzenit,
     $   alsurf,psurf,o3col,so2col,no2col,taucld,zbase,ztop,tauaer,
     $   ssaaer,alpha,dirsun,difdn,difup,zout,zaird,ztemp,lirrad,laflux,
     $   lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,iwfix,itfix,izfix,ns,
...
*=============================================================================*
      ...
      SUBROUTINE chkval(aerror,okvalu,oknew,inpfil,outfil,nstr,lat,lon,
     $   tmzone,iyear,imonth,iday,zstart,zstop,nz,wstart,wstop,nwint,
     $   tstart,tstop,nt,lzenit,alsurf,psurf,o3col,so2col,no2col,taucld,
     $   zbase,ztop,tauaer,ssaaer,alpha,dirsun,difdn,difup,zout,zaird,
     $   ztemp,lirrad,laflux,lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,
     $   iwfix,itfix,izfix,ns,ls,ims,slabel,nj,lj,imj,jlabel)
...
* check for legal dates:
      ...
      CALL calend(iyear,imonth,iday,jday,nday,oky,okm,okd)
      IF( .NOT. oky) THEN
         aerror = 'iyear'
         okvalu = .FALSE.
         RETURN
...
* Index for output at fixed altitude
      ...
      ENDIF
      IF(izfix .GT. 0) THEN
         zout = zstart+(zstop-zstart) *float(izfix-1)/float(nz-1)
...
*=============================================================================*
      ...
      SUBROUTINE  newval(avar,okfind,oktype,inpfil,outfil,nstr,lat,lon,
     $   tmzone,iyear,imonth,iday,zstart,zstop,nz,wstart,wstop,nwint,
     $   tstart,tstop,nt,lzenit,alsurf,psurf,o3col,so2col,no2col,taucld,
     $   zbase,ztop,tauaer,ssaaer,alpha,dirsun,difdn,difup,zout,zaird,
     $   ztemp,lirrad,laflux,lmmech,lrates,isfix,nms,ljvals,ijfix,nmj,
...
*  Internal
 ...
 40      CONTINUE
         ...
         WRITE(*,2000)
         WRITE(*,*)'Number = toggle T/F'
         WRITE(*,*)'n = next page'
         WRITE(*,*)'r = return to first page'
         WRITE(*,*)'<Enter> = done'
